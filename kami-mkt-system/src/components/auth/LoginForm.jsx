...
import { checkRateLimit, recordFailedAttempt } from '../../utils/rateLimiter';
...
export default function LoginForm() {
  ...
  async function handleLogin(e) {
    ...
    if (limit.blocked) {
      setError(`Muitas tentativas. Tente novamente em ${limit.minutesLeft} min.`);
      return;
    }
    ...
    try {
      ...
      setError(attemptsLeft > 0 ? `${msg} (${attemptsLeft} tentativa${attemptsLeft > 1 ? 's' : ''} restante${attemptsLeft > 1 ? 's' : ''})` : msg);
    } finally {
      setLoading(false);
    }
  }
